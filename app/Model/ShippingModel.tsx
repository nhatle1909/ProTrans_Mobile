...
import { GetShipList } from '@/Utils/ShippingAPI/ShippingAPI';
...
export const useShippingTaskList = () => {
  const [ShippingTaskList, setShippingTaskList] = useState<ShippingListModel[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await GetShipList(Token, DataToken.Id); // Assuming you have token and id
        ...
        const updatedData = await Promise.all(
          data.map(async (ShipTask) => {
            ...
            return { ...ShipTask, deadline: formattedDeadline, code: orderIdCut }; // Merge data
          })
        );
